...
public class PrimMSTVisualization extends JFrame {
    ...
    private final PrimStepper stepper;
    ...
    private final Color COLOR_MST_EDGE = new Color(39, 174, 96);
    ...
    private final String[] codeLines = {
            ...
            "    // T stores the vertices that have already been added to the MST.",
            "    let T be the set of vertices already added to the tree;",
            "",
            "    // cost[v] stores the cheapest known cost to connect vertex v to T.",
            "    // The starting vertex has cost 0, and all other vertices are",
            "    // initialized to INF.",
            "    set cost[s] = 0 and all other cost values to INF;",
            "",
            "    while size of T < number of vertices {",
            "        // Choose the vertex outside T that is cheapest to add next.",
            "        find u not in T with the smallest cost[u];",
            "",
            "        // Add this vertex to the growing MST.",
            "        add u to T;",
            "",
            "        // Check all edges from u to its neighboring vertices.",
            "        for each edge (u, v) from u {",
            "            // If v is outside T and edge (u, v) is cheaper than the current cost[v],",
            "            // update v's cost and parent.",
            "            if v is not in T and cost[v] > weight(u, v) {",
            "                cost[v] = weight(u, v);",
            "                parent[v] = u;",
            "            }",
            "        }",
    ...
    public PrimMSTVisualization() {
        setTitle("Prim MST Visualization - Java Version");
        setSize(1450, 900);
        ...
        stepper = new PrimStepper(vertices, edges, start);
    ...
    private void initGraph() {
        // The graph is made wider and taller to reduce overlap between edge weights and cost labels.
        addVertex("A", 390, 320);
        addVertex("B", 120, 320);
        addVertex("C", 390, 115);
        addVertex("D", 670, 320);
        ...
        addUndirectedEdge("A", "C", 6);
        addUndirectedEdge("A", "D", 5);
        addUndirectedEdge("A", "E", 9);

        addUndirectedEdge("B", "C", 2);
        addUndirectedEdge("C", "D", 3);
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("Prim's Minimum Spanning Tree Visualization");
    ...
    private String buildNarrative() {
        ...
        if (stepper.done) {
            ...
            sb.append("\nMeaning:\n");
            sb.append("- The green arrows are the edges selected into the minimum spanning tree.\n");
            sb.append("- cost[v] is the weight of the cheapest known edge connecting v to T.\n");
            sb.append("- The blue dashed line shows the current cheapest known connection from T to v.\n");
            sb.append("- parent[v] records which vertex in T gives that cheapest connection.\n");
        }
    ...
    private class GraphPanel extends JPanel {
        ...
        private void drawEdges(Graphics2D g2) {
            ...
            for (Edge e : edges) {
                ...
                // Do not highlight all outgoing edges from u in advance.
                // In the pseudocode, the "find u" step only chooses u.
                // An edge is highlighted only when the algorithm actually checks that edge.
        ...
        private void drawMstEdges(Graphics2D g2) {
            for (Edge e : getMstEdges()) {
                drawArrow(g2, e.u, e.v, COLOR_MST_EDGE, 4.4f);
            }
        }
        ...
        private void drawCostNearVertex(Graphics2D g2, Vertex v) {
            ...
            String parentText = stepper.parent.get(v) == null ? "-" : stepper.parent.get(v).id;
        ...
        private void drawLegend(Graphics2D g2) {
            ...
            drawLegendItem(g2, x, y, COLOR_IN_T, "In T");
            drawLegendItem(g2, x, y + 25, COLOR_CURRENT_U, "Current u");
            drawLegendItem(g2, x + 130, y, COLOR_CURRENT_V, "Current v");
            drawLegendLine(g2, x + 300, y - 6, COLOR_CHECKING_EDGE, "Checking edge", false, false);
            drawLegendLine(g2, x + 300, y + 19, COLOR_BEST_KNOWN, "Best known edge", false, true);
            drawLegendLine(g2, x + 500, y - 6, COLOR_MST_EDGE, "MST edge", true, false);
        }
    ...
    private class TablePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.drawString("cost[v] = cheapest known edge weight connecting v to T; parent[v] = vertex in T giving that edge.",
                    20, getHeight() - 10);
        }
    ...
    private String getStepExplanation(StepResult step) {
        switch (step.type) {
            case "init_T":
                return "Create T. T means the vertices already added to the growing MST.";

            case "init_cost":
                return "Start from A. Set cost[A] = 0. All other vertices are initialized to INF.";

            case "while_check":
                return "Check whether all vertices are already in T. If not, choose the next cheapest vertex to add.";

            case "find_min":
                return "Find u outside T with the smallest cost[u]. That means u is currently the cheapest vertex to add.";

            case "add_to_T":
                return "Add u to T. If parent[u] exists, the edge parent[u] -> u becomes a green MST edge.";

            case "for_prep":
                return "Now check all edges from u to its neighboring vertices outside T.";

            case "for_check":
                return "Check this edge (u, v). Its weight may give a cheaper way to connect v to T.";

            case "if_check":
                return "Check the if condition. If the edge is not cheaper than cost[v], skip the assignment lines and check the next edge.";

            case "update_cost":
                return "Update cost[v]. This edge is now the cheapest known way to connect v to T.";

            case "update_parent":
                return "Update parent[v] = u. This records which vertex in T gives the cheapest connection to v.";

            case "done":
                return "All vertices have been added to T. The green arrows form the minimum spanning tree.";

            case "init":
            default:
                return "Click Next Step to start Prim's algorithm from vertex A.";
        }
    }
    ...
    private static class PrimStepper {
        ...
        PrimStepper(List<Vertex> V, List<Edge> E, Vertex start) {
            this.V = V;
            this.E = E;
            this.start = start;
            reset();
        ...
        StepResult next() {
            if (done) {
                return new StepResult("done", 26, null, null);
            }

            switch (state) {
                case INIT_T:
                    trace.add("[Init] Create T as an empty set.");
                    state = State.INIT_COST;
                    return new StepResult("init_T", 2, null, null);

                case INIT_COST:
                    trace.add("[Init] Start from A. cost[A] = 0; all other costs are INF.");
                    state = State.WHILE_CHECK;
                    return new StepResult("init_cost", 7, start, null);

                case WHILE_CHECK:
                    if (T.size() < V.size()) {
                        trace.add("[While] T has " + T.size() + " of " + V.size() + " vertices. Continue.");
                        state = State.FIND_MIN;
                        return new StepResult("while_check", 9, null, null);
                    } else {
                        trace.add("[Done] T contains all vertices. Prim's algorithm is complete.");
                        state = State.DONE;
                        done = true;
                        return new StepResult("done", 26, null, null);
                    }
                ...
                case FIND_MIN:
                    u = findMinOutsideT();
                    trace.add("[Find u] Choose u = " + u.id + " because it has the smallest cost outside T: cost[" + u.id + "] = " + format(cost.get(u)) + ".");
                    state = State.ADD_TO_T;
                    return new StepResult("find_min", 11, u, null);

                case ADD_TO_T:
                    T.add(u);

                    if (parent.get(u) == null) {
                        trace.add("[Add to T] Add " + u.id + " to T. It is the start vertex, so no edge is added.");
                    } else {
                        Vertex p = parent.get(u);
                        trace.add("[Add to T] Add " + u.id + " to T. Add edge " + p.id + " -> " + u.id + " to the MST.");
                    }

                    state = State.FOR_PREP;
                    return new StepResult("add_to_T", 14, u, null);

                case FOR_PREP:
                    edgeIterator = outgoingEdgesFromU();
                    trace.add("[For] Check edges from u = " + u.id + " to vertices not in T.");
                    state = State.FOR_CHECK;
                    return new StepResult("for_prep", 17, u, null);
                ...
                case FOR_CHECK:
                    if (edgeIterator.hasNext()) {
                        ...
                        return new StepResult("for_check", 17, v, currentEdge);
                    } else {
                ...
                case IF_CHECK:
                    ...
                    if (!T.contains(v) && cost.get(v) > currentEdge.weight) {
                        trace.add("[Check] cost[" + v.id + "] is " + format(cost.get(v))
                                + ", edge weight is " + currentEdge.weight
                                + ". This edge is cheaper, so update.");
                        state = State.UPDATE_COST;
                    } else {
                        trace.add("[Check] No update for " + v.id + ". The if condition is false, so skip cost[v] and parent[v] assignments.");
                        state = State.FOR_CHECK;
                    }

                    return new StepResult("if_check", 20, v, currentEdge);

                case UPDATE_COST:
                    Vertex vCost = currentEdge.v;
                    cost.put(vCost, (double) currentEdge.weight);
                    trace.add("[Update cost] cost[" + vCost.id + "] = " + currentEdge.weight
                            + ". This is now the cheapest known edge from T to " + vCost.id + ".");
                    state = State.UPDATE_PARENT;
                    return new StepResult("update_cost", 21, vCost, currentEdge);

                case UPDATE_PARENT:
                    Vertex vParent = currentEdge.v;
                    parent.put(vParent, u);
                    trace.add("[Update parent] parent[" + vParent.id + "] = " + u.id
                            + ". So the current best connection to " + vParent.id + " is from " + u.id + ".");
                    state = State.FOR_CHECK;
                    return new StepResult("update_parent", 22, vParent, currentEdge);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            PrimMSTVisualization app = new PrimMSTVisualization();
            app.setVisible(true);
        });
    }
}
